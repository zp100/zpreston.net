...
function load_video() {
    ...
    // Update page title.
    document.title = `${all_songs[key].title} | ${username}'s tracks | YouTune`
...
// Sends the current user data to the database as a "fetch" request.
function send_to_database() {
    // Stop if the user is a guest.
    if (username === '<guest>') return

    // Compile the user's song list into data.
    const body = {
        'username': username,
        'user_data': {
            'all_songs': all_songs,
            'song_key_list': song_key_list,
    ...
    // Send the request to the server as a POST.
    fetch(database_url, options)
}



// Sends the queue and recent lists to the server as a "fetch" request.
function send_to_extra_lists() {
    // Stop if the user is a guest, or didn't select to save extra lists.
    if (username === '<guest>' || !save_extra) return

    // Compile the user's extra lists into data.
    const body = {
        'queue_key_list': queue_key_list,
        'recent_key_list': recent_key_list,
    }
    ...
    // Send the request to the server as a POST.
    fetch(extra_lists_url, options)
}



// Sends the password change to the server as a "fetch" request.
function send_to_password(password, new_password) {
    // Stop if the user is a guest.
    if (username === '<guest>') return

    // Compile the password change into data.
    const body = {
        'password': password,
        ...
        .then(text => {
            ...
            if (text !== '') {
                ...
                if (password_error_el) {
                    // Update the error.
                    password_error_el.innerHTML = text
                } else {
                    // Create the error if its element doesn't exist.
                    const new_password_el = document.querySelector('input[name="new-password"]')
                    new_password_el.insertAdjacentHTML('afterend', `<i name="password-error" class="text-danger">${text}</i>`)
                }
            } else {
                // Remove the error.
                const password_error_el = document.querySelector('i[name="password-error"]')
...
// Sends the account info to the server as a "fetch" request.
function send_to_login(mode, username, password) {
    // Compile the account info into data.
    const body = {
        'mode': mode,
        'username': username,
        'password': password,
    ...
    fetch(login_url, options)
        ...
        .then(text => {
            ...
            if (text !== '') {
                ...
                if (login_error_el) {
                    // Update the error.
                    login_error_el.innerHTML = text
                } else {
                    // Create the error if its element doesn't exist.
                    const new_password_el = document.querySelector('input[name="password"]')
                    new_password_el.insertAdjacentHTML('afterend', `<i name="login-error" class="text-danger">${text}</i>`)
                }
            } else {
                // Reload the page.
